...
function Article({ 
  ...
  const {onDeleteSubmit} = useContext(AppContext);
  ...
  return (
    ...
    <Card style={{ width: '18rem' }}>
      ...
      <Card.Body>
        ...
        {isOwner && <Link to={`/${blog_id}/edit`} className="button"><Button className="mt-1" >Edit</Button></Link>}
        <div>{authValues.isAuthenticated && <Link to={`/${blog_id}`} className="button">Comments</Link>}</div>
